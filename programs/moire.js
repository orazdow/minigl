const fs = /*glsl*/`#version 300 es
	...
	float sdf2(vec3 v){
		// float a = sin(time)*.2+.22;
		float a = sin(time)*cos(.8*time)+cos(4.+1.2*time)*.3;
		return sdBox(sin((7.*v)+time), a*.2+.23);
	}
	...
	vec3 light(vec3 p, vec3 l, vec3 c, float a){
		float lt = max(dot(normalize(l),normal(p)),.1);
		return pow(lt,a)*c;
	}
